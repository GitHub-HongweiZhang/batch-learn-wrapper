...
int model_command::run() {
    ...
    if (val_file_name.empty()) { // No validation set given, just train
        ...
        double minimum_loss = 1.0;
        uint best_epoch = 0;
        for (uint epoch = 0; epoch < n_epochs; ++ epoch) {
            cout << "Epoch " << epoch << "..." << endl;

            train_on_dataset(*model, ds_train);
            auto loss = evaluate_on_dataset(*model, ds_val);
        ...
        if (early_stopping == 1)
            std::cout << "get minimum loss " << std::fixed << std::setprecision(5) << minimum_loss << " at epoch " << best_epoch << std::endl;
    }
